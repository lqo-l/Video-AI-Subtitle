...
form.onsubmit=async event=>{event.preventDefault();message.textContent="保存中…";const data=Object.fromEntries(fields.map(x=>[x,document.getElementById(x).value]));data.api_key=api_key.value===KEY_PLACEHOLDER?"":api_key.value;try{const r=await fetch(`${API}/config`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(data)});if(!r.ok)throw new Error((await r.json()).detail);const saved=await r.json();message.textContent="已保存";if(saved.api_key_configured){api_key.value=KEY_PLACEHOLDER;api_key.dataset.configured="true";}chrome.runtime.sendMessage({type:"release-service"});}catch(e){message.textContent=`失败：${e.message}`;}};
// Moon End
