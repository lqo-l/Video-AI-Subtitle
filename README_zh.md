...
- **运行库故障降级** —— “自动”模式下即使 CUDA 在识别迭代时才发现 cuBLAS/cuDNN 缺失，也会提示并切换 CPU 继续。
- **可选 GPU 配置** —— Whisper 默认使用 CPU；用户可在“更多设置”中主动下载插件私有 CUDA 运行库，全程显示组件、大小、速度与进度。
...
- **高级本机设置** —— 可指定多语言 Whisper 路径并自动检查，选择镜像/官方源，安装时提前下载并查看大小、速度和百分比。
- **完整任务控制** —— 支持暂停、继续和取消；取消后保留已有提取、翻译与摘要进度。
- **统一布局与外观** —— 覆盖/挤压同时控制普通页面和全屏，常用按钮分组展示，面板背景透明度可调。
- **工作区式挤压布局** —— 挤压时网站顶部栏与内容同步让位，侧栏使用全高分隔轨道、品牌标题和卡片式状态区。
- **适配 B 站顶栏** —— 普通页面自动避开当前 B 站导航栏，全屏时再恢复为全高侧栏。
- **安静的 B 站入口** —— B 站不再弹出处理确认框，仅显示可拖拽并记忆位置的“译”悬浮按钮。
...
打开英文或日文 YouTube/Bilibili 视频，点击扩展图标，选择**处理当前视频**。公开字幕会被优先复用；否则插件下载音频并由本机 Whisper 自动识别语言。首次使用某个 Whisper 模型时会优先通过 `hf-mirror.com` 下载并显示百分比，镜像不可用时自动回退官方源；下载完成后复用本机缓存。英文专用的 `.en` 模型不支持日语，因此首次启用日语时仍需下载对应的多语言模型。首批中文字幕生成后即可播放，摘要和关键点并行产生。
