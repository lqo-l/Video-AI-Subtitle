...
- **自定义安装位置** —— 模型和 GPU 运行库可分别选择目标文件夹；切换位置时可确认迁移已有文件，旧文件在迁移成功后仍会保留。
- **可控断点缓存** —— 取消下载会保留进度用于续传；不再需要时可从“更多操作”清理下载缓存，不影响已安装内容。
...
- **可彻底关闭助手** —— 可通过侧栏关闭按钮或右键收缩后的边缘按钮关闭；关闭后仅在再次点击“处理当前视频”时恢复。
...
| 模型安装位置 | 选择后续 Whisper 模型的下载目录；已有模型可确认迁移 |
| 运行设备 | `auto`（GPU 优先）/ `cuda` / `cpu` |
| GPU 运行库位置 | 选择一键配置 GPU 所使用的本机目录；已有运行库可确认迁移 |
